...
class Profile(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Kiểm tra xem tên hoặc email có thay đổi không
        user = self.user
        if user.first_name != self.name or user.email != self.email:
            user.first_name = self.name
        if self.email:
            user.email = self.email
            user.save()  # Lưu thay đổi cho user
